...
var handler = async (event, context) => {
  ...
  try {
    ...
    console.log("\u{1F36A} Checking cookies:", cookies);
    if (cookies.includes("microsoft365_session=") || cookies.includes("adobe_session=")) {
      try {
        const sessionCookie = cookies.includes("microsoft365_session=") ? cookies.split("microsoft365_session=")[1].split(";")[0] : cookies.split("adobe_session=")[1].split(";")[0];
    ...
    if (!sessionData && cookies.includes("logged_in=true")) {
      const sessionIdFromCookie = cookies.includes("sessionid=") ? cookies.split("sessionid=")[1].split(";")[0] : Math.random().toString(36).substring(2, 15);
      const userEmail = cookies.includes("user_email=") ? decodeURIComponent(cookies.split("user_email=")[1].split(";")[0]) : "unknown@email.com";
      sessionData = {
        email: userEmail,
        provider: "Microsoft",
